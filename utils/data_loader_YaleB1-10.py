...
def load_orl_dataset():
    ...
    for subject in sorted(os.listdir(data_path)):
        ...
        for subject2 in sorted(os.listdir(subject_dir)):
            subject_dir2 = os.path.join(subject_dir, subject2).replace('\\', '/')
            if not os.path.isdir(subject_dir2):
                continue

            print(f"处理文件夹: {subject_dir2}")

            for image_file in sorted(os.listdir(subject_dir2)):
                # 更灵活的文件扩展名处理
                if not image_file.lower().endswith(('.bmp', '.png', '.jpg', '.jpeg')):
                    continue

                image_path = os.path.join(subject_dir2, image_file).replace('\\', '/')
                try:
                    ...
                    y.append(int(subject[-2:]) - 1)  # s1 -> 0, s2 -> 1等
